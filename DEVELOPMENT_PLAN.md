...
- **우선순위**: ⭐⭐⭐ (높음)
- **복잡도**: 중간
- **설명**: 공모주 청약일정, 실적발표, 배당 기준일 등 투자 이벤트를 월별 캘린더 뷰로 통합 표시
- **가치**: 날짜 기반 이벤트 한눈에 파악, 투자 일정 관리 용이
- **구현**:
  - [ ] 월별 캘린더 그리드 컴포넌트 개발 (주 단위 7열, 5~6행)
  - [ ] 공모주 청약일정 캘린더 통합 (`/api/ipo-calendar` 연동)
  - [ ] 실적 발표일 캘린더 통합 (`/api/earnings-calendar` 연동)
  - [ ] 이벤트 타입별 색상 구분 (공모주/실적/배당)
  - [ ] 날짜 클릭 시 해당일 이벤트 상세 팝오버
  - [ ] 이전/다음 달 네비게이션 + 오늘 버튼
  - [ ] 이벤트 없는 날 최소화, 이벤트 많은 날 뱃지 표시
...
### 스프린트 1 (우선순위 최상)
1. [ ] **캘린더 형식 UI** — 공모주·실적·배당 이벤트 월별 통합 캘린더 뷰
2. [ ] 맞춤형 알림 시스템
3. [ ] 실시간 포트폴리오 추적

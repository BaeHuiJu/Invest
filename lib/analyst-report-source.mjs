const US_STOCKS = [
  ...
  // Tech
  'AMD', 'INTC', 'QCOM', 'CRM', 'ORCL', 'ADBE', 'NOW', 'MU', 'AMAT',
  // AI/Cloud
  'PLTR', 'SNOW', 'NET', 'DDOG', 'PANW',
  // Financial
  'BRK-B', 'JPM', 'V', 'BAC', 'GS', 'MS', 'C', 'AXP', 'BLK', 'SCHW', 'BX',
  // Healthcare
  'UNH', 'LLY', 'JNJ', 'ABBV', 'MRK', 'PFE', 'AMGN', 'GILD',
  // Consumer
  'WMT', 'PG', 'KO', 'PEP', 'NKE', 'MCD', 'SBUX', 'HD', 'COST', 'TGT',
  // Energy
  'XOM', 'CVX', 'COP', 'SLB',
  // Industrial/Defense
  'CAT', 'DE', 'BA', 'HON', 'LMT', 'RTX',
  // Media/Telecom
  'NFLX', 'DIS', 'T', 'VZ',
];
...
export function calculateBaseGapPct(report) {
  if (!(report.basePrice > 0) || !(report.targetPrice > 0)) {
    return 0;
  }

  return roundOne(((report.targetPrice - report.basePrice) / report.basePrice) * 100);
}

export function getBasePriceReferenceLabel(report) {
  if (report.basePriceDate) {
    return `${report.basePriceDate} 종가`;
  }
...
async function fetchHistoricalCloseByMarket(ticker, market, targetDate, priceCache) {
  if (market === 'korea') {
    return fetchNaverHistoricalClose(ticker, targetDate, priceCache);
  }

  return fetchYahooHistoricalClose(ticker, targetDate, priceCache);
}

function buildPendingPerformancePoint(asOfDate) {
  return {
    asOfDate,
    closePrice: 0,
    returnPct: 0,
...
function buildUnavailablePerformancePoint(asOfDate) {
  return {
    asOfDate,
    closePrice: 0,
    returnPct: 0,
...
async function buildPerformancePoint(report, days, priceCache) {
  const asOfDate = formatDate(addDays(report.date, days));
  const now = new Date();
  now.setHours(0, 0, 0, 0);

  if (new Date(asOfDate) > now) {
    return buildPendingPerformancePoint(asOfDate);
  }

  if (!(report.basePrice > 0) || !(report.targetPrice > 0)) {
    return buildUnavailablePerformancePoint(asOfDate);
  }

  const historicalClose = await fetchHistoricalCloseByMarket(report.ticker, report.market, asOfDate, priceCache);
  const closePrice = historicalClose?.price || 0;
  if (!(closePrice > 0)) {
    return buildUnavailablePerformancePoint(asOfDate);
  }

  const returnPct = roundOne(((closePrice - report.basePrice) / report.basePrice) * 100);
  const initialDistanceToTarget = Math.abs(report.targetPrice - report.basePrice);
  const currentDistanceToTarget = Math.abs(report.targetPrice - closePrice);
  const rawTargetProgressPct = initialDistanceToTarget === 0
    ? (closePrice === report.targetPrice ? 100 : 0)
    : (1 - (currentDistanceToTarget / initialDistanceToTarget)) * 100;
  const targetProgressPct = roundOne(clamp(rawTargetProgressPct, 0, 100));

  return {
    asOfDate: historicalClose.date || asOfDate,
    closePrice: roundTwo(closePrice),
    returnPct,
    targetProgressPct,
    success: targetProgressPct >= 70,
    status: 'complete',
  };
}

export async function enrichReportsWithPerformance(reports) {
  const priceCache = new Map();

  return Promise.all(reports.map(async (report) => ({
    ...report,
    performance: {
      week1: await buildPerformancePoint(report, 7, priceCache),
      month1: await buildPerformancePoint(report, 30, priceCache),
      month3: await buildPerformancePoint(report, 90, priceCache),
    },
  })));
}
...
export function buildReasonBullets(report) {
  ...
  if (report.basePrice > 0) {
    bullets.push(`${getBasePriceReferenceLabel(report)} ${formatPriceByMarket(report.basePrice, report.market)} 대비 괴리는 ${calculateBaseGapPct(report)}%입니다.`);
  }
...
async function fetchNaverResearch(days) {
  ...
  try {
    const maxPages = days >= 365 ? 40 : days >= 180 ? 20 : days >= 90 ? 10 : 3;
...
export function buildStockInsight(reportGroup) {
  ...
  const valuationBullets = getUniqueBullets([
    ...
    latest.basePrice > 0 && latest.targetPrice > 0 ? `${getBasePriceReferenceLabel(latest)} 대비 목표가 괴리는 ${calculateBaseGapPct(latest)}%입니다.` : '',
  ]);
...
export async function buildAnalystCacheFile(maxDays = 365) {
  const baseReports = await buildAnalystReports(maxDays);
  const reports = await enrichReportsWithPerformance(baseReports);

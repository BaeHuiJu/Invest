...
- **완료일**: 2026-05-12
- **구현**:
  - [x] 과거 변동성 데이터 수집 (Yahoo Finance / Naver Finance 3개월 일별 종가)
  - [x] 리스크 지표 계산 로직 (베타·변동성·샤프·VaR 95%)
  - [x] 시장/리스크 수준 집중도 분포 바
  - [x] 리스크 점수 시각화 (0-100 원형 게이지)
- **파일**:
  - `pages/api/risk-analysis.ts` - 리스크 지표 계산 API (10분 캐시)
  - `components/RiskAnalysisTab.tsx` - 리스크 분석 탭 UI
  - `lib/dashboard-config.ts` - `'risk-analysis'` 탭 추가
  - `pages/index.tsx` - 렌더링 분기 추가
- **지표 계산**:
  - **베타**: Cov(종목, 벤치마크) / Var(벤치마크) — 국내 ^KS11, 해외 ^GSPC
  - **변동성**: std(일별수익률) × √252 (연간화)
  - **샤프 비율**: (연간수익률 - 3%) / 연간변동성
  - **VaR 95%**: -(평균 - 1.645 × std) × 100 (1일 최대손실 추정)
  - **리스크 점수**: 변동성(40pt) + 베타(30pt) + VaR(30pt) 합산
- **특징**:
  - 최근 30일 추천 종목 자동 수집 (최대 15개)
  - 포트폴리오 요약: 평균 베타·변동성·샤프·VaR + 시장/리스크 분포 바
  - 종목 카드: 원형 게이지 + 저/중/고위험 배지 + 4개 지표 그리드
  - 정렬: 리스크점수 / 변동성 / 샤프비율 / 베타
  - 필터: 전체 / 고위험 / 중위험 / 저위험

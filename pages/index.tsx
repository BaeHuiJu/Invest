  ...
  return <>
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="mx-auto max-w-7xl px-4 py-6">
        {activeTab === 'analyst'
          ? <AnalystTab onOpenInsight={setInsightTarget} />
          : loading
            ? <LoadingState />
            : error
          ...
          <div className="space-y-2.5 p-3 md:hidden">
            {paginated.map((report, index) => <article key={`${report.market}-${report.ticker}-${index}`} className="rounded-lg border border-gray-100 bg-gray-50 p-3">
              <div className="flex items-start justify-between gap-2.5">
                <button type="button" onClick={() => onOpenInsight({ ticker: report.ticker, name: report.name, market: report.market, currentPrice: report.currentPrice })} className="min-w-0 text-left">
                ...
                <span className={`shrink-0 rounded px-1.5 py-0.5 text-[11px] ${report.market === 'korea' ? 'bg-blue-100 text-blue-700' : 'bg-orange-100 text-orange-700'}`}>{report.market === 'korea' ? '국내' : '해외'}</span>
              </div>
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y">
                {paginated.map((report, index) => <tr key={`${report.market}-${report.ticker}-${index}`} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3"><button type="button" onClick={() => onOpenInsight({ ticker: report.ticker, name: report.name, market: report.market, currentPrice: report.currentPrice })} className="text-left"><div className="font-medium text-blue-700 hover:underline">{report.name}</div><div className="text-xs text-gray-400">{report.ticker}</div></button></td>
      ...
      {stocks.length === 0 ? <div className="p-8 text-center text-gray-400">데이터가 없습니다.</div> : filtered.length === 0 ? <div className="p-8 text-center text-gray-400">검색 결과가 없습니다.</div> : <>
        <div className="space-y-3 p-4 md:hidden">
          {sorted.map((stock) => <article key={stock.ticker} className="rounded-xl border border-gray-100 bg-gray-50 p-4">
            <button type="button" onClick={() => onOpenInsight({ ticker: stock.ticker, name: stock.name, market, currentPrice: stock.currentPrice, changePercent: stock.changePercent, high52w: stock.high52w, low52w: stock.low52w })} className="w-full text-left">
              <div className="flex items-start justify-between gap-3">
                ...
                <div className={`shrink-0 text-sm font-semibold ${stock.changePercent >= 0 ? 'text-green-600' : 'text-red-600'}`}>{stock.changePercent >= 0 ? '+' : ''}{stock.changePercent.toFixed(2)}%</div>
              </div>
...
function StockInsightModal({ request, onClose }: { request: InsightRequest | null; onClose: () => void }) {
  ...
  return <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40 px-3 py-3 sm:items-center sm:px-4 sm:py-6" onClick={onClose}>
    <div className="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-t-3xl bg-white shadow-xl sm:max-h-[90vh] sm:rounded-2xl" onClick={(event) => event.stopPropagation()}>
      <div className="flex items-start justify-between gap-4 border-b p-4 sm:p-5"><div className="min-w-0"><h2 className="truncate text-lg font-semibold text-gray-900 sm:text-xl">{request.name}</h2><p className="text-sm text-gray-500">{request.ticker} · {request.market === 'korea' ? '국내' : '해외'}</p></div><button type="button" onClick={onClose} className="shrink-0 rounded-lg px-3 py-1 text-sm text-gray-500 hover:bg-gray-100">닫기</button></div>
